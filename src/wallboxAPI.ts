...
export class wallboxAPI {
    ...
    getRaw(path: string, payload: any, options: any) {
        ...
        return new Promise((resolve, reject) => {
            var req = https.request({
                ...
                console.error("Low-level error", e);
                reject(501);
            });
    ...
    async authenticate() {
        var r: any = await this.getRaw("/auth/token/user", null, {headers: {authorization: "Basic " + Buffer.from(this.config.username + ":" + this.config.password).toString("base64")}}).catch((err)=>{
            return {};
        });
    ...
    async get(path: string, payload: any, options: any) {
        ...
        if (this.jwt) {
            var r: any = await this.getRaw(path, payload, {...{headers: {authorization: "Bearer " + this.jwt}, ...options}}).catch((err)=>{
                return null;
            });
